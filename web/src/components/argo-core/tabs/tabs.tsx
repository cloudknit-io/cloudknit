			...
			<div className="tabs" ref={container => (this.container = container)}>
				<div
					...
					<div className={classNames({ 'fixed-width': this.props.fixed })}>
						<div className={classNames('tabs__nav-wrapper')}>
							{/* TODO changed a href to div in order to fix issue with missing href (check if it works V.T.)*/}
							{this.props.tabs.map(tab => (
								<div
									key={tab.key}
									onClick={() => this.selectTab(tab)}
									className={classNames({ active: this.isTabSelected(tab) })}>
				...
				{selectedTab && (
					<div
						...
						style={{
							height: selectedTab.isOnlyContentScrollable
								? `calc(100vh - ${selectedTab.extraVerticalScrollPadding || 0}px)`
								: 'inherit',
