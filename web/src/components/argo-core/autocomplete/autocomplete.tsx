...
export const Autocomplete = (props: AutocompleteProps) => {
	...
	React.useEffect(() => {
		const listener = () => {
			// TODO check this part (V.T.)
			// Recalculate menu position on scroll
			// if (
			// 	autocompleteEl &&
			// 	autocompleteEl.refs.input &&
			// 	autocompleteEl.refs.menu &&
			// 	!(event.target === autocompleteEl.refs.menu)
			// ) {
	...
	return (
		<ReactAutocomplete
						...
						el.setMenuPositions = () => {
							...
							if (el.refs.menu && el.refs.input) {
								...
								if (window.innerHeight - (menuTop + el.refs.menu.offsetHeight) < 0) {
									const correctedTop =
										menuTop - el.refs.menu.offsetHeight - el.refs.input.offsetHeight;
									if (correctedTop > 0) {
			...
			renderMenu={function (menuItems, _, style) {
				...
				return (
					<div style={{ ...style, ...{}, background: 'white', zIndex: 10, maxHeight: '20em' }}>
						{menuItems}
					</div>
				);
			}}
