...
export class PopupManager implements PopupApi {
	...
	public confirm(title: string, message: string | React.ComponentType): Promise<boolean> {
		...
		return new Promise(resolve => {
			...
			this.popupPropsSubject.next({
				title: (
					<span>
						{title} <i className="argo-icon-close" onClick={() => closeAndResolve(false)} />
					</span>
				),
				content,
				footer: (
					<div>
						<button
							qe-id="argo-popup-ok-button"
							className="argo-button argo-button--base"
							onClick={() => closeAndResolve(true)}>
							OK
						</button>{' '}
						<button
							qe-id="argo-popup-cancel-button"
	...
	// TODO implement types (V.T.)
	renderForm = (settings: any, defaultValues: any, onSubmit: any, formApi: any, form: any) => (
		<Form
			validateError={settings && settings.validate}
			defaultValues={defaultValues}
			onSubmit={onSubmit}
			getApi={api => (formApi = api)}>
			{api => <form onSubmit={api.submitForm}>{form(api)}</form>}
		</Form>
	);
	...
	public prompt(
		...
		return new Promise(resolve => {
			...
			this.popupPropsSubject.next({
				...
				title: (
					<span>
						{title} <i className="argo-icon-close" onClick={() => closeAndResolve(null)} />
					</span>
				),
				titleColor: titleColor ? titleColor : 'normal',
				icon: customIcon ? { name: customIcon?.name, color: customIcon?.color } : undefined,
				content: () => this.renderForm(settings, defaultValues, onSubmit, formApi, form),
				footer: (
					<div>
						<button
							qe-id="prompt-popup-ok-button"
							className="argo-button argo-button--base"
							onClick={e => formApi.submitForm(e)}>
							OK
						</button>{' '}
						<button
							qe-id="prompt-popup-cancel-button"
							className="argo-button argo-button--base-o"
