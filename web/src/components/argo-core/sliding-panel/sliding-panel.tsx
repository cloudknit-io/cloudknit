...
export const SlidingPanel = (props: SlidingPanelProps) => (
	<div
		className={classNames('sliding-panel', {
		...
		<div className="sliding-panel__wrapper">
			...
			{props.header && (
				<div
					className={classNames('sliding-panel__header', {
						'sliding-panel__header--close-btn-right-padding': props.hasCloseButton,
					})}>
					{props.header}
				</div>
			)}
			<div className="sliding-panel__body">{props.children}</div>
			{props.footer && <div className="sliding-panel__footer">{props.footer}</div>}
		</div>
